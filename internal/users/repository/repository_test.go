	...
	}{
		{
			...
			mockSetup: func() {
				poolMock.ExpectExec(query).
					WithArgs(testUser.UUID, testUser.Username, testUser.Email, testUser.Password, testUser.Role).
...
func TestGetUserByUUID(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Success": {
			uuid: testUUID,
			mockSetup: func() {
				rows := pgxmock.NewRows([]string{"uuid", "username", "email", "password", "role", "created_at", "updated_at"}).
					AddRow(expectedUser.UUID, expectedUser.Username, expectedUser.Email, expectedUser.Password,
						expectedUser.Role, expectedUser.CreatedAt, expectedUser.UpdatedAt)
		...
		"Success": {
			...
			mockSetup: func() {
				...
				poolMock.ExpectQuery(updateQuery).
					WithArgs(inputUser.Username, inputUser.Email, inputUser.Role, inputUser.UUID).
					WillReturnRows(rows)
			},
...
func TestGetAllUsers(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Success Multiple Users": {
			mockSetup: func() {
				rows := pgxmock.NewRows([]string{"uuid", "username", "email", "password", "role", "created_at", "updated_at"}).
					AddRow(user1.UUID, user1.Username, user1.Email, user1.Password, user1.Role, user1.CreatedAt, user1.UpdatedAt).
					AddRow(user2.UUID, user2.Username, user2.Email, user2.Password, user2.Role, user2.CreatedAt, user2.UpdatedAt)
			...
			checkResult: func(t *testing.T, users []*models.User, err error) {
				...
				assert.Equal(t, user2.Password, users[1].Password)
				assert.Equal(t, user2.Role, users[1].Role)
			},
		},
